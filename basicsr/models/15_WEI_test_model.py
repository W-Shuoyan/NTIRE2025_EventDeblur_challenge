...
class WEI_test_model(BaseModel):
    def __init__(self, opt):
        super(WEI_test_model, self).__init__(opt)
        ...
        # # load pretrained models
        # load_path_default = self.opt['path'].get('pretrain_network_g', None)
        # load_path_finetune = self.opt['path'].get('pretrain_network_g_finetune', None)

        # if self.image_name.startswith("zigzag") and load_path_finetune:
        #     load_path = load_path_finetune
        # else:
        #     load_path = load_path_default
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger,
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            load_path = self.opt['path'].get('pretrain_network_g', None)
            if self.image_name.startswith("zigzag"):
                load_path = self.opt['path'].get('pretrain_network_g_finetune', None)
            if load_path is not None:
                self.load_network(self.net_g, load_path,
                                    self.opt['path'].get('strict_load_g', True),
                                    param_key=self.opt['path'].get('param_key', 'params'))
